// set bash title
process.stdout.write("\x1b]2;Goat Bot V2 - Fca By SaGor\x1b\x5c");
...
async function startBot(loginWithEmail) {
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			try {
				// convert to promise
				const item = await axios.get("https://raw.githubusercontent.com/SAGOR-KINGx/all-around/refs/heads/main/gban.json");
				dataGban = item.data;
			...
			let notification;
			try {
				const getNoti = await axios.get("https://raw.githubusercontent.com/SAGOR-KINGx/all-around/refs/heads/main/notification.txt");
			...
			console.log(`\x1b[1m\x1b[33m${("COPYRIGHT:")}\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36m${("Project GoatBot v2 created by SaGor (https://github.com/SAGOR-KINGx), please do not sell this source code or claim it as your own. Thank you!")}\x1b[0m`);
			logColor("#f5ab00", character);
